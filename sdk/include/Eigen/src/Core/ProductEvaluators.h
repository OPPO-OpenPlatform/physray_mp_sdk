    ...
    : evaluator_base<Product<Lhs, Rhs, LazyProduct> >
{
  ...
  enum {
    ...
    CoeffReadCost = InnerSize==0 ? NumTraits<Scalar>::ReadCost
                  ...
                  : InnerSize * (int(NumTraits<Scalar>::MulCost) + int(LhsCoeffReadCost) + int(RhsCoeffReadCost))
    ...
    CanVectorizeInner =    SameType
                        && LhsRowMajor
                        && (!RhsRowMajor)
                        && (int(LhsFlags) & int(RhsFlags) & int(ActualPacketAccessBit))
                        && (int(InnerSize) % int(packet_traits<Scalar>::size) == 0)
  };
