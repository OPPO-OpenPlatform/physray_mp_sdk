...
// the below #if in both the cpp and the matching one in the header.
// You will also need to implement some of the TODO code.
#if false

GLTFSceneReader::GLTFSceneReader(
...
std::shared_ptr<const SceneAsset> GLTFSceneReader::read(const std::string & assetPath) {
    ...
    gltf::GLTFSceneAssetBuilder sceneBuilder(_assetSystem, _textureCache, _world, &model, assetBaseDirectory, _skinnedMeshes, _morphTargetManager);

    // Generate all of the available scenes and fetch the result.
    PH_LOGI("[GLTF] Building scene graph....");
    std::shared_ptr<SceneAsset> sceneAsset = sceneBuilder.build(_scenes, _mainScene);
