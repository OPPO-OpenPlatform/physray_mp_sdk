...
Timeline::Timeline(): _time(), _duration(), _rate(1.0), _playCount(0), _repeatCount(1) {
    //
}
...
void Timeline::tick(std::chrono::duration<uint64_t, std::nano> elapsedTime) {
    ...
    // If rate is negative.
    if (_rate < 0.0) {
        // Calculate the positive equivelant to rate.
        ...
        // If tick would bring us before 0.
        if (scaledElapsedTime > _time) {
            // If we still had some repeats previous tick.
            if (hasRepeatsLeft()) {
                // Calculate how far before the beginning time has gone.
                std::chrono::duration<uint64_t, std::nano> timeBeforeStart = scaledElapsedTime - _time;
                ...
                _playCount = _playCount + 1 + (uint32_t) (timeBeforeStart / _duration);
                ...
                if (hasRepeatsLeft()) {
                    ...
                    nextTime = _duration - timeBeforeStart % _duration;

                    // If we used up all of our repeats.
                } else {
                    // Then clamp time to zero.
                    nextTime = std::chrono::duration<uint64_t, std::nano>::zero();
                }
            ...
            // If the next time would still be in range [0..duration].
        } else {
            // Calculate next time.
        ...
        if (nextTime > _duration) {
            ...
            if (hasRepeatsLeft()) {
                ...
                _playCount = _playCount + 1 + (uint32_t) (timeAfterEnd / _duration);

                // If we still have repeats this tick after using one or more up.
                if (hasRepeatsLeft()) {
                    // Wrap time around from the beginning by how much after the end we went.
                    // Just in case we somehow looped around multiple times, use the remainder.
                    nextTime = timeAfterEnd % _duration;
