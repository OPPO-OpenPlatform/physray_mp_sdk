...
static inline Eigen::Vector3f limitRotationAngle(const Vector3f & a) {
    return {
        std::clamp(a.x(), -HALF_PI, HALF_PI),
        a.y(), // no limiting on Yaw
        std::clamp(a.z(), -HALF_PI, HALF_PI),
    };
}
...
FirstPersonController & FirstPersonController::setTargetPosition(const Vector3f & p) {
    _positionInterp.reset(_position, p, INTERP_TIME);
...
FirstPersonController & FirstPersonController::setTargetOrbitalRadius(float r) {
    _orbitalRadiusInterp.reset(_orbitalRadius, r, INTERP_TIME);
    return *this;
}
...
FirstPersonController & FirstPersonController::setOrbitalRadius(float r) {
    ...
    _orbitalRadius = std::max(_minimalRadius, r);
...
FirstPersonController & FirstPersonController::setTargetAngle(const Vector3f & r) {
    _rotationInterp.reset(_angle, r, INTERP_TIME);
...
void FirstPersonController::flythroughUpdate(float elapsedSeconds) {
    ...
    if (!delta_p.isZero()) {
        ...
        if (_flythroughBoundary.isEmpty() || _flythroughBoundary.contains(targetPosition)) { setTargetPosition(targetPosition); }
    }
