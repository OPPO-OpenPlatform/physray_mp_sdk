...
GLTFMaterialBuilder::GLTFMaterialBuilder(TextureCache * textureCache, ph::rt::World * world, const tinygltf::Model * model,
                                         const std::vector<ph::RawImage> * images)
    : _textureCache(textureCache), _world(world), _model(model), _images(images) {
...
ph::rt::Material * GLTFMaterialBuilder::build(const tinygltf::Material & material) {
    ...
    for (std::size_t index = 0; index < 3; ++index) {
        phMaterialDesc.albedo[index] = (float) metallicRoughness.baseColorFactor[index];
    }
    ...
    for (std::size_t index = 0; index < 3; ++index) {
        phMaterialDesc.emission[index] = (float) material.emissiveFactor[index];
    }
    ...
    auto phMaterial  = _world->createMaterial(phMaterialDesc);
    phMaterial->name = material.name;
    return phMaterial;
}
...
ph::rt::Material::TextureHandle GLTFMaterialBuilder::getOrmTextureHandle(const tinygltf::Material & material) {
    ...
    if (iterator != _ormToTextureHandle.end()) {
        ...
        channels[3].setDefaultValue(255);
