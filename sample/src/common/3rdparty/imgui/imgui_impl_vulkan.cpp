...
void ImGui_ImplVulkan_SetMinImageCount(uint32_t min_image_count)
{
    ...
    ImGui_ImplVulkan_InitInfo* v = &g_VulkanInitInfo;
    VkResult err = ph::va::threadSafeDeviceWaitIdle(v->Device);
    check_vk_result(err);
    ImGui_ImplVulkanH_DestroyWindowRenderBuffers(v->Device, &g_MainWindowRenderBuffers, v->Allocator);
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    wd->Swapchain = 0;
    err = ph::va::threadSafeDeviceWaitIdle(device);
    check_vk_result(err);
...
{
    ph::va::threadSafeDeviceWaitIdle(device); // FIXME: We could wait on the Queue if we had the queue in wd-> (otherwise VulkanH functions can't use globals)
