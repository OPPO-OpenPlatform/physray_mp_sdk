...
TextureCache::TextureCache(ph::va::VulkanSubmissionProxy * vsp, ph::AssetSystem * assetSystem): _vsp(vsp), _assetSystem(assetSystem) {}

ph::rt::Material::TextureHandle TextureCache::loadFromAsset(const std::string & assetPath) {
    ...
    imageObject.createFromImageProxy(assetPath.c_str(), *_vsp, VK_IMAGE_USAGE_SAMPLED_BIT, ph::va::DeviceMemoryUsage::GPU_ONLY, asset.content.i.proxy());
...
void TextureCache::buildExportData(ph::rt::JediPbrt3Exporter & exporter) {
    auto iterator = _textureHandles.begin();
    while (iterator != _textureHandles.end()) {
        exporter.addTexture(iterator->second, iterator->first);
        iterator++;
    }
}
...
ph::rt::Material::TextureHandle TextureCache::createShadowMap2D(const char * name, VkFormat format, uint32_t size) {
    ...
    ph::va::BufferObject<VK_BUFFER_USAGE_TRANSFER_SRC_BIT, ph::va::DeviceMemoryUsage::CPU_ONLY> sb;
...
ph::rt::Material::TextureHandle TextureCache::createShadowMap2D(const char * name) { return createShadowMap2D(name, VK_FORMAT_R32_SFLOAT, 512); }
...
ph::rt::Material::TextureHandle TextureCache::createShadowMapCube(const char * name, VkFormat format, uint32_t size) {
    ...
    shadowMap.create(name, _vsp->vgi(),
                     ph::va::ImageObject::CreateInfo {}.setCube(size).setFormat(format).setUsage(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
...
ph::rt::Material::TextureHandle TextureCache::createShadowMapCube(const char * name) { return createShadowMapCube(name, VK_FORMAT_R32_SFLOAT, 512); }
