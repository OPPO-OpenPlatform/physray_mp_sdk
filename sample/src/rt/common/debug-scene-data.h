...
struct LightDebug { // Stores pointers needed to construct and update a debug mesh visualizing a light
    sg::Node *         lightMeshNode;
    ph::rt::Model *    lightModel;
    ph::rt::Material * lightMat;
    bool               enabled = false;
};

typedef std::map<const sg::Node *, LightDebug> LightDebugMap;
...
struct SceneDebugManager {
    ...
    SceneDebugManager() {}
    SceneDebugManager(ph::rt::World * w, sg::Graph * g, ph::rt::Mesh * sm, ph::rt::Mesh * dm, ph::rt::Mesh * qm)
        : world(w), scene(&g->scene()), graph(g), sphereMesh(sm), diskMesh(dm), quadMesh(qm) {}

    // sg::Transform transformFromLight(const ph::rt::Light * light) {
    //     sg::Transform tfm;
    //     auto &                ld = light->desc();
    ...
    //         Eigen::Vector3f direction = (ld.type == ph::rt::Light::Type::DIRECTIONAL) ? ld.directional.direction : ld.spot.direction;
    ...
    // sg::Node * createDebugNode(const ph::rt::Light * light, bool enabled) {
    //     sg::Transform zeroScaled;
    //     zeroScaled.setScaling(Eigen::Vector3f::Zero());
    //     sg::Transform tfm = enabled ? transformFromLight(light) : zeroScaled;
    //     // First create the node, then set the transform
    //     auto node = graph->createNode({.parent = light->nodes()[0]});
    ...
    //     auto mat = world->create(light->name, mcp);
    //     return mat;
    // }
    ...
    LightDebug & initLightDebug(const sg::Node * light) {
        LightDebug ld = {
    ...
    bool * getDebugEnable(const sg::Node * light) {
    ...
    void setDebugEnable(const sg::Node * light, bool enable) {
        auto toSet = getDebugEnable(light);
        *toSet     = enable;
        if (enable) updateDebugLight(light);
    }

    void onDeleteLight(const sg::Node * light) {
        auto debugDataItr = lightDebugData.find(light);
        if (debugDataItr != lightDebugData.end()) {
            LightDebug & debugData = debugDataItr->second;
            graph->deleteNodeAndSubtree(debugData.lightMeshNode);
        }
        lightDebugData.erase(light);
    }

    void updateDebugLight(const sg::Node * light) {
        (void) light;
