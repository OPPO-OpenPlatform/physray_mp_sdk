 ...
 *
 * File : first-person-controller.h
 *
 * Version: 2.0
 *
 * Date : Feb 2023
...
public:
    ...
    FirstPersonController & setMoveSpeed(float sx) {
        _moveSpeed = {sx, sx, sx};
        return *this;
    }
    ...
    FirstPersonController & setHandness(Camera::Handness handness) {
        _handednessMultiplicationFactor = (Camera::RIGHT_HANDED == handness ? 1.0f : -1.0f);
        return *this;
    }
...
private:
    Eigen::Vector2f _pitchLimits = {-PI, PI};
    Eigen::Vector2f _rollLimits  = {-PI, PI};
...
public:
    ...
    FirstPersonController & setPitchLimits(Eigen::Vector2f minMaxPitch) {
    ...
    FirstPersonController & setRollLimits(Eigen::Vector2f minMaxRoll) {
        float limit = orbiting() ? PI : HALF_PI;
...
private:
    ...
    float                          _maximalRadius = std::numeric_limits<float>::max();
...
private:
    ...
    Eigen::Vector3f limitRotationAngle(const Eigen::Vector3f & a);
};
